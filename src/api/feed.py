...
from src.storage.models import Release, ReleaseTmdbTitle, ReleaseTpdbScene, TpdbScene, TpdbSite
...
async def load_feed_releases(
    ...
    """Re-fetch releases with TMDB/TPDB metadata and source side-table eager-loaded."""
    ids = [r.id for r in releases]
    if not ids:
        return []
    opts = [
        selectinload(Release.tmdb_titles).selectinload(ReleaseTmdbTitle.tmdb_metadata),
        selectinload(Release.tpdb_scenes)
        .selectinload(ReleaseTpdbScene.scene)
        .selectinload(TpdbScene.site)
        .selectinload(TpdbSite.network),
    ]
...
def tpdb_for(release: Release):
    return release.tpdb_scenes[0].scene if release.tpdb_scenes else None
